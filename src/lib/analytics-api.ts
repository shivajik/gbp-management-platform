...
export class AnalyticsService {
  ...
  static async getAnalyticsData(
    ...
    try {
      ...
      if (businessProfileId) {
        ...
        businessProfiles = allBusinessProfiles.filter((profile: any) => 
          profile.attributes?.selectedForAnalytics === true
        );
      }
      ...
      const locationComparison = this.createLocationComparison(businessProfiles);
   ...
   */
  static async syncInsightsData(userId: string, businessProfileId?: string): Promise<void> {
    try {
      ...
      console.log(`Syncing insights for ${businessProfiles.length} locations...`);
   ...
   */
  private static calculateOverviewMetrics(businessProfiles: any[]): AnalyticsMetrics {
   ...
   */
  private static generateTrendData(businessProfiles: any[], dateRange: { start: Date; end: Date }) {
    const trends = {
    ...
    Array.from(insightsByDate.entries()).forEach(([dateKey, insights]) => {
      const totalViews = insights.reduce((sum: number, insight: any) => sum + (insight.totalViews || 0), 0);
      const totalSearches = insights.reduce((sum: number, insight: any) => sum + (insight.totalSearches || 0), 0);
      const totalActions = insights.reduce(
        (sum: number, insight: any) => 
          sum + (insight.websiteClicks || 0) + 
   ...
   */
  private static createLocationComparison(businessProfiles: any[]): LocationComparison[] {
    return businessProfiles.map(profile => ({
      locationId: profile.id,
      locationName: profile.name,
      metrics: {
        totalViews: profile.insights?.reduce((sum: number, insight: any) => sum + (insight.totalViews || 0), 0) || 0,
        totalSearches: profile.insights?.reduce((sum: number, insight: any) => sum + (insight.totalSearches || 0), 0) || 0,
        websiteClicks: profile.insights?.reduce((sum: number, insight: any) => sum + (insight.websiteClicks || 0), 0) || 0,
        phoneCallClicks: profile.insights?.reduce((sum: number, insight: any) => sum + (insight.phoneCallClicks || 0), 0) || 0,
        directionRequests: profile.insights?.reduce((sum: number, insight: any) => sum + (insight.directionRequests || 0), 0) || 0,
        photoViews: profile.insights?.reduce((sum: number, insight: any) => sum + (insight.photoViews || 0), 0) || 0,
        averageRating: profile.reviews?.length > 0 
          ? profile.reviews.reduce((sum: number, review: any) => sum + review.rating, 0) / profile.reviews.length 
          : 0,
  ...
  private static getTopPerformingPosts(businessProfiles: any[]) {
    const allPosts = businessProfiles.flatMap(profile => 
    ...
    return allPosts
      ...
      .sort((a, b) => (b.metrics.views + b.metrics.clicks) - (a.metrics.views + a.metrics.clicks))
      .slice(0, 5)
      .map(post => ({
        id: post.id,
  ...
  private static getRecentReviews(businessProfiles: any[]) {
    const allReviews = businessProfiles.flatMap(profile => 
